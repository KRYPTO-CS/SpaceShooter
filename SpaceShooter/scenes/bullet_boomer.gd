...
func _ready():
	...
	AudioManager.play_sound(preload("res://sounds/boomerShoot.wav"), 0.3, volume_mult) # shot sound
	...
	match element_type:
		...
		_:
			sprite.material.set_shader_parameter("new_palette", load("res://sprites/bullets/palettes/paletteBoomer.png"))
...
func _process(delta):
	...
	if not returning:
		...
		current_velocity = current_velocity.move_toward(-velocity*2, 2500 * delta)
		position += current_velocity * delta

	# When lifetime reached, reverse direction instead of freeing
	if time_alive >= lifetime * LIFETIME_MULT and not returning:
		current_velocity = velocity
		returning = true
		time_alive = 0.0
	elif returning and time_alive >= lifetime * 5 * LIFETIME_MULT:
		queue_free()
...
func _on_area_entered(area):
	if area.is_in_group("destroyable"):
		...
		AudioManager.play_sound(preload("res://sounds/boomerHit.wav"), 0.3, volume_mult)
		match element_type:
			GameManager.ElementType.FIRE:
